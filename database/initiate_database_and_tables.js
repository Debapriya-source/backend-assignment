...
const create_customer_data_query = `CREATE TABLE customer_data (customer_id INT PRIMARY KEY, first_name VARCHAR(50) NOT NULL, last_name VARCHAR(50) NOT NULL, age INT, phone_number INT, monthly_salary DECIMAL(20, 2), approved_limit DECIMAL(20, 2), current_debt DECIMAL(20, 2));`;
const describe_customer_data_query = `describe customer_data`;
const create_loan_data_query = `CREATE TABLE loan_data (customer_id INT, loan_id INT PRIMARY KEY, loan_amount DECIMAL(20, 2), tenure INT, interest_rate DECIMAL(5, 2), monthly_repayment DECIMAL(20, 2), EMIs_paid_on_time INT, start_date DATE, end_date DATE, FOREIGN KEY (customer_id) REFERENCES customer_data(customer_id));`;
